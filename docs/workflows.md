...
Local video-production workflow with 40 nodes. It receives news through a webhook and orchestrates Ollama, ComfyUI, Persian TTS, FFmpeg, intro/outro processing, background music, validation, cleanup, and Telegram delivery.
...
2. Activate its production webhook or expose it through an authenticated tunnel.
3. Set `GERMAN_NEWS_VIDEO_WEBHOOK_URL` for the hosted n8n instance.
